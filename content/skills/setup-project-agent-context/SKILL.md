...
description: Use this skill when the user wants to set up AIE OS, configure agent context, or generate agent instruction files such as AGENTS.md or CLAUDE.md for a project. It gathers missing configuration, runs the AIE OS CLI, and verifies the generated artifacts.
---
...
- Generate the agent instructions file (`AGENTS.md`, or the name given to `--output-file`) through the AIE OS CLI.
...
3. Ask one short decision at a time unless grouping closely related choices is clearer. For example:
   ...
   - ask which name the generated instructions file should use, defaulting to `AGENTS.md` and suggesting `CLAUDE.md` for Claude Code;
...
12. Run `docker compose -f aie-os/docker-compose.yaml run --rm aie-os build --output-file <approved-file-name>`. If the build refuses because the file was not generated by AIE OS, report it and ask before re-running with `--force-overwrite`.
13. Verify `.aie-os/aie-os.json`, `.aie-os/build/effective-context.json`, and a non-empty agent instructions file (`AGENTS.md`, or the approved `--output-file` name).
...
- Do not write, patch, or reconstruct the agent instructions file or `.aie-os/build/effective-context.json` directly.
